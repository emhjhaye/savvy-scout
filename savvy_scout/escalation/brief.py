...
def _section_table(col_headers: list[str], rows: list[tuple], col_widths: list[float], styles) -> Table:
    ...
    data = [header]
    for row in rows:
        data.append([Paragraph(str(cell) if cell not in (None, "") else "&mdash;", styles["BriefCell"]) for cell in row])
...
def _header_block(notice: sqlite3.Row, banner_text: str, styles) -> list:
    meta_bits = [notice["buyer"] or "UNVERIFIED", f"Reference: {notice['ref']}"]
    if notice["sector"]:
        meta_bits.append(f"Sector: {notice['sector']}")
    return [
        Paragraph(banner_text, styles["BriefBanner"]),
        Paragraph(notice["title"] or "UNVERIFIED", styles["BriefTitle"]),
...
def build_internal_addendum(conn: sqlite3.Connection, notice_id: int, output_dir: str = DEFAULT_BRIEFS_DIR) -> str:
    ...
    story.append(_callout_box(
        ...
        "outcomes, scouting rationale, open blockers, and open questions. It must not be shared with "
        "Trifork or any external party. AUTO-GENERATED PROVISIONAL DRAFT FOR VALIDATION.",
        styles,
    ))
    ...
    gate_summary_rows = (
        [(g["gate_name"], f"{g['outcome']}. {g['reason']}") for g in gate_rows]
        or [("Not yet triaged", "")]
    )
    ...
    # Notice link is deliberately its own row, never dropped -- the actual
    # published notice on its source portal (Find a Tender, etc.), not just
    # the reference number.
    scouting_rows = [
        ("Source", notice["source"]),
        ("First seen", (notice["first_seen_at"] or "")[:10] or "UNVERIFIED"),
    ...
    story.append(Paragraph("C. CAPABILITY FIT (PHASE 2 AI READ)", styles["BriefSectionHeading"]))
    if assessment:
        fit_rows = [
            ...
            ("Competitor position", f"{assessment['competitor_position_rating']} -- "
             f"{assessment['competitor_position_reasoning']} (PROVISIONAL, FOR VALIDATION)"),
            ("Overall", f"{assessment['overall_rating']} -- "
             f"{assessment['overall_reasoning']} (PROVISIONAL, FOR VALIDATION)"),
        ]
    else:
        fit_rows = [("Phase 2 scope read", "Has not run yet for this notice.")]
    story.append(_section_table(["Dimension", "Assessment"], fit_rows, [4 * cm, 12 * cm], styles))
    ...
    story.append(Paragraph("E. OPEN QUESTIONS FOR VICTORIA", styles["BriefSectionHeading"]))
    ...
    story.append(_callout_box(
        f"Decision required: go, no-go or park for &ldquo;{notice['title']}&rdquo; (ref {notice['ref']}).",
        styles,
    ))
...
def build_capture_brief(conn: sqlite3.Connection, notice_id: int, output_dir: str = DEFAULT_BRIEFS_DIR) -> str:
    ...
    story.append(Paragraph(notice["award_criteria_summary"] or "Not stated.", styles["BriefCell"]))
    ...
    story.append(Paragraph((notice["text_blob"] or "UNVERIFIED")[:1200], styles["BriefCell"]))
    ...
    fit_text = (
        f"{assessment['overall_rating']} -- {assessment['overall_reasoning']}"
